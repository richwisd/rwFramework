...
class rwCache
{
    ...
    public static function connect(array $options = [], $name = false)
    {
        ...
        $className=$type.'CacheClass';
        if (false === $name) {
            $name = md5(serialize($options));
        }
       
        if (true === $name || !isset(self::$instance[$name])) {
            $classExtFile=FRAME_PATH.'rwCache/driver/'.$type.'.php';
            if (!is_file($classExtFile)){
                    $classExtFile=FRAME_PATH.'rwCache/driver/File.php';
            }

            include_once $classExtFile;
