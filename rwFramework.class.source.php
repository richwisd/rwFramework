...
if (!isset($comm['TIME_ZONE'])) {
	$comm = include CONF_PATH . 'comm.php';
}
$timeZone = $comm['TIME_ZONE'];
date_default_timezone_set($timeZone);
unset($timeZone);
define('START_TIME', microtime(true));
define('PAGE_TIME', $_SERVER['REQUEST_TIME']);
//加载系统全局函数，请注意不要再另外添加其他文件的全局函数，可以修改此文件，添加函数，但是添加之前一定要注意是否已经存在相同功能的函数
include FRAME_PATH . 'application.base.php';
//加载数据库类，此处并没有加载数据库，在需要的时候才会加载
require FRAME_PATH . 'db.class.php';
//加载系统缓存驱动，此处并没有使用到系统缓存，在需要的时候才会用到
require_once FRAME_PATH . 'rwCache/Driver.php';

/**
 *    KZWMS框架核心文件，包含最基础的类与函数
 *    @author    枯桥断道
 */
/*---------------------以下是系统常量-----------------------*/
// 记录页面时间
...
class rwFramework {
	...
	function start($app) {
		define('APP_NAME', $app);
		/* 加载初始化文件 */
		define('APP_PATH', ROOT_PATH . APP_NAME . '/app/');
		include APP_PATH . 'app.base.php';
		require_once FRAME_PATH . 'model.base.php';
		/* 数据过滤 */
		$pm = preg_replace('/(\W+)/', '', _G('p', 'string', 'index'));
		$pm == '' ? '' : 'index';
		$act = preg_replace('/(\W+)/', '', _G('a', 'string', 'index'));
		$act == '' ?: 'index';
		define('PM', $pm);
		define('ACT', $act);
		$appFile = APP_PATH . PM . '.app.php';

		if (is_file($appFile)) {
			require $appFile;
			define('MODEL', '');
			$className = ucfirst($pm);
			/* 实例化控制器 */
			$appClass = new $className($app);
			if ($act[0] != '_' && method_exists($appClass, $act)) {
				$appClass->$act();
			}
		} else {
			//判断model.app.php是否存在
			//存在，则自动引入model.app.php
			$appFile = APP_PATH . "model.app.php";
			if (is_file($appFile)) {
				require $appFile;
				define('MODEL', '');
...
class Base {
	var $openSmarty = false;
	var $memcachedOpened = false;
	var $redisOpened = false;
	var $rwCacheOpened = false;
	var $isSession = false;
	var $_lang = array(); //语言包数据
	var $_cfg; //配置文件数据
	var $_behaviorData; //用户行为数据
	var $_beginTime; //页面开始时间，毫秒级
	var $logFile;
	var $tpl;
	var $runClient;
	var $otherDB;
